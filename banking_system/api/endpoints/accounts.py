...
@router.post("/create", tags=["Accounts"])
...
@router.put("/update/{account_id}", tags=["Accounts"])
...
@router.delete("/delete/{account_id}", tags=["Accounts"])
...
@router.get("/get/{account_id}", tags=["Accounts"])
...
@router.get("/statement/{account_id}", tags=["Accounts"])
...
@router.post("/transfer", tags=["Accounts"])
...
@router.post("/payment", tags=["Accounts"])
def make_your_payment(
    payer_account_id: int = Form(...),
    payee_account_id: int = Form(...),
    amount: float = Form(...),
    db: Session = Depends(get_db),
    token_data: TokenData = Depends(JWTBearer())
):
    result = make_payment(db, payer_account_id, payee_account_id, amount)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result
